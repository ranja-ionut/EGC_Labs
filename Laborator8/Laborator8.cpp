...
void Laborator8::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(lightPosition.x / 3, lightPosition.y / 3, lightPosition.z / 3));
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, 0.5f, 25));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 1, 0));
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0, 0.5, 0));
	}
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, 0.5f, 50));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 1, 0));
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0, 0.5, 0));
	}
}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	glUniform1i(glGetUniformLocation(shader->program, "spot"), spot);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		// BONUS 1
		if (window->KeyHold(GLFW_KEY_1)) lightDirection = glm::rotate(glm::mat4(1), deltaTime * 5.f, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 0);
		if (window->KeyHold(GLFW_KEY_2)) lightDirection = glm::rotate(glm::mat4(1), -deltaTime * 5.f, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 0);
		if (window->KeyHold(GLFW_KEY_3)) lightDirection = glm::rotate(glm::mat4(1), deltaTime * 5.f, glm::vec3(0, 0, 1)) * glm::vec4(lightDirection, 0);
		if (window->KeyHold(GLFW_KEY_4)) lightDirection = glm::rotate(glm::mat4(1), -deltaTime * 5.f, glm::vec3(0, 0, 1)) * glm::vec4(lightDirection, 0);
		if (window->KeyHold(GLFW_KEY_5)) angle += deltaTime * 10;
		if (window->KeyHold(GLFW_KEY_6)) angle -= deltaTime * 10;
	}
}
