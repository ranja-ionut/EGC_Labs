...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-1, 0.5f, 2));
...

	{
		...
		modelMatrix = glm::rotate(modelMatrix, RADIANS(33.33f), glm::vec3(0, 0, 1));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(33.33f), glm::vec3(0, 1, 0));
		RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
	}
	...
	if (perspective) {
		projectionMatrix = glm::perspective(RADIANS(60) + fov, window->props.aspectRatio, 0.01f, 200.0f);
	}
	else {
		projectionMatrix = glm::ortho(-3.f - width, 3.f + width, -3.f - height, 3.f + height, -100.f, 100.f);
	}
}
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (perspective) {
		...
		if (window->KeyHold(GLFW_KEY_C)) {
			width += deltaTime * 0.2f;
			height += deltaTime * 0.4f;
		}
...
void Laborator5::OnKeyPress(int key, int mods)
{
	...
	{
		perspective = false;
		projectionMatrix = glm::ortho(-3.f - width, 3.f + width, -3.f - height, 3.f + height, -25.f, 100.f);
	}
